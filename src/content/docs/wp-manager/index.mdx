...
Claude Code can help you develop, test, and maintain WP Manager through its complete development lifecycle.

### Development Setup

Ask Claude to set up your development environment:

> Set up the WP Manager development environment
...
# Set up environment
cp .env.example .env.local
# Claude will prompt for TURSO_DATABASE_URL and TURSO_AUTH_TOKEN
...
Claude can run the test suite and interpret results:
...
If tests fail, share the output with Claude for diagnosis.

### Adding New Features

Tell Claude what you need:

> Add a "backup status" field to the sites table that shows when each site was last backed up

Claude will:
1. Update the Drizzle schema in `src/db/schema.ts`
2. Generate and run the migration
3. Update TypeScript types
4. Add UI components to display backup status
5. Write tests for the new feature

### Database Operations

Ask Claude to help with database tasks:

> Show me all sites that haven't been synced in 24 hours

Claude uses Drizzle Studio or writes queries:
```bash
npm run db:studio
```

Or generates queries:
```typescript
const staleSites = await db.query.sites.findMany({
  where: lt(sites.lastSyncedAt, new Date(Date.now() - 86400000))
})
```
...
> I'm getting "Failed to fetch" when syncing a site. Here's the network tab: [paste error]

Claude will:
- Check CORS configuration
- Verify REST API endpoint
- Test application password format
- Suggest fixes for connector plugin issues

### Code Reviews

Ask Claude to review your changes:

> Review this PR for security issues: [paste diff or file]
...
Tell Claude about slow operations:

> The dashboard takes 5 seconds to load with 50 sites
...
Claude can help with Vercel deployment:
...
Claude diagnoses build errors and environment variable issues.
