...
If you have a Claude Code Max subscription, you may still need to provide an API key for gateway authentication:
...
While Claude Code Max handles authentication for direct Anthropic requests, the gateway requires explicit credentials to forward requests. If you experience 401 errors, ensure `ANTHROPIC_AUTH_TOKEN` is set.
