---
title: Claude Code Setup
description: Custom Claude Code configuration with agents, commands, and workflows
source_project: /Users/jb
sidebar:
  order: 0
---

import Mermaid from '../../../components/Mermaid.astro';

This documents the custom Claude Code setup used across JB Cloud projects, including slash commands, agents, and automated workflows.

<Mermaid title="Claude Code Architecture">
{`flowchart TB
    subgraph Config["~/.claude/"]
        CLAUDE[CLAUDE.md]
        subgraph Commands["commands/"]
            NewProj[new-project.md]
            Plan[plan.md]
            TDD[tdd.md]
            End[end.md]
        end
        subgraph Agents["agents/"]
            Planner[planner.md]
            Reviewer[code-reviewer.md]
            Security[security-reviewer.md]
        end
        subgraph Rules["rules/"]
            Style[coding-style.md]
            Git[git-workflow.md]
            Test[testing.md]
        end
        subgraph Skills["skills/learned/"]
            Patterns[*.md]
        end
    end

    User[User] -->|claude| CC[Claude Code]
    CC --> CLAUDE
    CLAUDE --> Commands
    CLAUDE --> Agents
    CLAUDE --> Rules
    CC -->|/learn| Skills
`}
</Mermaid>

## Quick Start

All configuration lives in `~/.claude/`:

```text
~/.claude/
├── CLAUDE.md           # Global instructions
├── commands/           # Slash commands
├── agents/             # Specialized agents
├── skills/             # Skills and learned patterns
│   └── learned/        # Auto-extracted patterns
├── rules/              # Modular rules
└── contexts/           # Saved session contexts
```

## Available Resources

| Resource | Description |
|----------|-------------|
| [Commands](/claude-code/commands/) | All slash commands reference |
| [Agents](/claude-code/agents/) | Specialized agent definitions |
| [Workflows](/claude-code/workflows/) | Common development workflows |

## Key Commands

<Mermaid title="Command Categories">
{`flowchart LR
    subgraph Lifecycle["Project Lifecycle"]
        NP[/new-project]
        P[/plan]
        T[/tdd]
        E[/end]
    end

    subgraph Git["Git & Releases"]
        CRL[/create-release]
        SGA[/setup-github-actions]
    end

    subgraph Docs["Documentation"]
        JD[/jbdocs]
    end

    subgraph Session["Session Management"]
        CS[/context-save]
        CR[/context-restore]
        R[/remind]
    end

    subgraph Quality["Quality"]
        Rev[/code-review]
        DC[/deploy-check]
        DA[/deps-audit]
    end
`}
</Mermaid>

### Project Lifecycle

```bash
/new-project              # Initialize new project
/plan                     # Create implementation plan
/tdd                      # Test-driven development
/end                      # End session cleanly
```

### Git & Releases

```bash
/create-release           # Create release with git tag
/create-release --patch   # Auto-increment patch version
/setup-github-actions     # Configure CI/CD workflows
```

### Documentation

```bash
/jbdocs                   # Sync to docs.jbcloud.app
/jbdocs commands          # Update commands reference
```

### Session Management

```bash
/save-to-notion           # Save session context to Notion (auto or manual)
/context-save             # Save session state (local)
/context-restore          # Resume session
/remind                   # Quick context reminder
```

### Quality

```bash
/code-review              # Review code quality
/deploy-check             # Pre-deployment checklist
/deps-audit               # Audit dependencies
```

## Philosophy

<Mermaid title="Development Philosophy">
{`flowchart TD
    A[New Task] --> B{Complex?}
    B -->|Yes| C[Plan Mode]
    B -->|No| D[Direct Implementation]
    C --> E[Delegate to Agents]
    E --> F[Parallel Execution]
    F --> G[TDD Approach]
    D --> G
    G --> H[Security Review]
    H --> I[Document]
    I --> J[Complete]
`}
</Mermaid>

1. **Agent-First**: Delegate complex tasks to specialized agents
2. **Parallel Execution**: Use Task tool with multiple agents when possible
3. **Plan Before Execute**: Use Plan Mode for complex operations
4. **Test-Driven**: Write tests before implementation
5. **Security-First**: Never compromise on security
6. **Document Everything**: Sync to docs.jbcloud.app automatically
7. **Save Before Compact**: Auto-save to Notion at edit thresholds to prevent context loss

## Configuration

Global preferences in `~/.claude/CLAUDE.md`:
- No emojis in code or docs
- Prefer immutability
- Many small files over few large
- Conventional commits
- 80% minimum test coverage

## Repository

GitHub: [Aventerica89/jb-cloud-docs](https://github.com/Aventerica89/jb-cloud-docs)
