...
```
  ...
  <Fragment slot="tab-2">
```bash
pnpm create astro@latest jb-cloud-docs -- --template starlight
cd jb-cloud-docs
pnpm install
```
  </Fragment>
  <Fragment slot="tab-3">
```bash
yarn create astro jb-cloud-docs --template starlight
cd jb-cloud-docs
yarn install
```
  </Fragment>
...
<RelatedDocs links={[
  { href: '/documentation-workflow/', label: 'Documentation Workflow', description: 'How to document processes in real-time with Claude Code' },
  { href: '/changelog/', label: "What's New", description: 'Latest updates and features' }
]} />
